...
from networks.helpers import initialize_weights
...
class ResNet(nn.Module):
    def __init__(self, in_channels=4, output_stride=16, backbone='resnet101', pretrained=False):
...
class Xception(nn.Module):
    def __init__(self, output_stride=16, in_channels=4, pretrained=False):
        ...
        # Middle Flow
        for i in range(16):
            exec('self.block'+(str(i+4))+ '= Block(728, 728, stride=1, dilation=mf_d)')
        ...
        for k, v in pretrained_weights.items():
            if k in state_dict:
                ...
                if k.startswith('block11'):
                    ...
                    model_dict[k] = v
                    for i in range(8):
                        model_dict[k.replace('block11', 'block'+(i+12))] = v
...
class DeepLab_RGBD(BaseModel):
    def __init__(self, num_classes, in_channels=4, backbone='xception', pretrained=False,
                output_stride=16, freeze_bn=False, freeze_backbone=False, **_):
                
        super(DeepLab_RGBD, self).__init__()
