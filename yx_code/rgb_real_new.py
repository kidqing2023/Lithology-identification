...
from sklearn.metrics import accuracy_score, confusion_matrix, precision_score, roc_curve, recall_score, auc, \
...
CELL_Y = 256
row_cell = 10000
col_cell = 10000   #按照自己测试图像的行列数进行修改
NetName = 'FCN8'  # unet，FCN8,DeepLab
CLASS_NUM = 7
...
def predict_img(net,
    ...
    x, y = full_img.shape[0], full_img.shape[1]
    slice = zoom(full_img, (256 / x, 256 / y, 1), order=0)
    slice = np.transpose(slice, (2, 0, 1))        #交换坐标轴
    input = torch.from_numpy(slice).unsqueeze(
        0).float().cuda()
    net.eval()
    with torch.no_grad():
...
def writeTiff(image, fileName, geotrans, proj):
    ...
    dataset_result = driver.Create(fileName, col, row, 1, gdal.GDT_Byte)
    dataset_result.SetGeoTransform(geotrans)  # 写入仿射变换参数
    dataset_result.SetProjection(proj)  # 写入投影1
    dataset_result.GetRasterBand(1).WriteArray(image)
...
def identify_single_img(image_name, dsm_name, model_path, row, col):
    ...
    # image = np.concatenate((image, hsm), axis=2)     #FCN、DeepLab、unet不需要用到DSM
    rowheight = int(row)
    colwidth = int(col)
...
if __name__ == '__main__':
    ...
    model_path = 'D:/2022yanxing/20220505/20220505/FCN8_best_model.pth'
    # Uncertainty_Aware_Mean_Teacher_3_labeled
    # Fully_Supervised_3_labeled
    images_path = 'D:/2022yanxing/0526/FCN/realmask/dom'
    dsms_path = 'D:/2022yanxing/0526/FCN/realmask/dsm'
    row = 256
    col = 256
    timeall = 0
